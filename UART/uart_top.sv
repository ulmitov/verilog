...
module uart_top (
    ...
    uart #(
        ...
        .lcreg(lcr),
        .fcreg(fcr),
        .divisor({dlm, dll}[`UART_DIV_WIDTH-1:0]),
        .wr_data(data_in),
        .rd_uart(rbr_rd),
        .wr_uart(thr_wr),
        .rx_ext(sin),
    ...
    assign wen = cs & wr;
    assign ren = cs & rd;
    assign thr_wr = wen && addr == `UART_REG_THR;
    assign rbr_rd = ren && addr == `UART_REG_RBR;
    assign rx_pull = rbr_rd & ~rx_empty;
    assign data_bus = rx_pull ? rd_data : {`UART_DATA_WIDTH{1'bZ}};
