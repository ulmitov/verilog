...
class base_sequence extends uvm_sequence#(transaction);
    ...
    virtual task post_start();
        // Send a packet with disabled push and pull
        super.post_start();
		start_item(req);
        if (!req.randomize() with { pull == 0; push == 0; })
    ...
    virtual task seq_parallel(input int repeats = 1);
        ...
        repeat(repeats) begin
            start_item(req);
            if (!req.randomize() with { push == 1; pull == 1; })
                uvm_report_error(get_name(), "Failed to randomize");
            finish_item(req);
        end
    endtask
...
class sequence_rand extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** RANDOM SEQUENCE START ***"));
        seq_random(default_repeats);
        uvm_report_info(get_name(), $sformatf("*** RANDOM SEQUENCE END ***"));
    endtask
endclass
...
class sequence_parallel extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** PARALLEL SEQUENCE START ***"));
        ...
        uvm_report_info(get_name(), $sformatf("*** PARALLEL SEQUENCE END ***"));
    endtask
endclass
...
class sequence_consecutives_while_empty extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** CONSECUTIVE WHILE EMPTY SEQUENCE START ***"));
        seq_pull(fifo_config::FIFO_DEPTH);
        ...
        uvm_report_info(get_name(), $sformatf("*** CONSECUTIVE WHILE EMPTY SEQUENCE END ***"));
    endtask
endclass
...
class sequence_consecutives_while_full extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** CONSECUTIVE WHILE FULL SEQUENCE START ***"));
        ...
        uvm_report_info(get_name(), $sformatf("*** CONSECUTIVE WHILE FULL SEQUENCE END ***"));
    endtask
endclass
...
class sequence_push_pull_00 extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** PUSH+PULL 0x00 SEQUENCE START ***"));
        default_repeats = 2;
        ...
        uvm_report_info(get_name(), $sformatf("*** PUSH+PULL 0x00 SEQUENCE END ***"));
    endtask
endclass
...
class sequence_push_pull_ff extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** PUSH+PULL 0xFF SEQUENCE START ***"));
        default_repeats = 2;
        ...
        uvm_report_info(get_name(), $sformatf("*** PUSH+PULL 0xFF SEQUENCE END ***"));
    endtask
endclass
...
class sequence_push_pull_FF_00 extends base_sequence;
    ...
    task body;
        uvm_report_info(get_name(), $sformatf("*** PUSH+PULL 0xFF+0x00 SEQUENCE START ***"));
        ...
        uvm_report_info(get_name(), $sformatf("*** PUSH+PULL 0xFF+0x00 SEQUENCE END ***"));
    endtask
endclass
